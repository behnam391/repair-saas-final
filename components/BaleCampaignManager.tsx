...
import BaleContactDirectory from "./BaleContactDirectory";
import BaleDeliveryReport from "./BaleDeliveryReport";
export default function BaleCampaignManager() {
  ...
  async function action(action: string, id?: string) {
    ...
    try {
      const r = await fetch("/api/superadmin/bale-campaigns", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, id, audience, message, phone, selectedPhones: selectionMode ? selectedPhones : undefined }) });
