...
export type Insight = { tone: string; icon: string; title: string; detail: string };
const ICONS: Record<string, any> = { clock: Clock3, stock: Boxes, message: MessageCircle, profit: Banknote, star: Star };

export default function MorningInsights({ suppliedItems }: { suppliedItems?: Insight[] }) {
  const [items, setItems] = useState<Insight[]>([]);
  const [open, setOpen] = useState(true);
  useEffect(() => { if (suppliedItems !== undefined) { setItems(suppliedItems); return; } fetch("/api/dashboard/insights").then(r => r.json()).then(d => setItems(d.insights ?? [])).catch(() => {}); }, [suppliedItems]);
