...
const Schema = z.object({
  // Both the number and the OTP get retyped on a Farsi keyboard; neither
  // would match otherwise. See lib/phone.ts.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const attemptLimit = await rateLimit(`reset-password:${clientIp(req)}:${phone}`, 6, 10 * 60 * 1000);
    ...
    await revokeSessionsForSubject(LoginSubjectKind.SHOP_USER, user.id, {
