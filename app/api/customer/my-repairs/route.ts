...
export async function GET() {
  try {
    ...
    const tickets = await db.ticket.findMany({
      ...
      take: 100,
      select: {
        id: true, no: true, deviceModel: true, deviceCategory: true, deviceType: true, deviceBrand: true, status: true, lane: true,
        ...
        invoice: { select: { id: true, total: true, paid: true, paidAmount: true } },
      },
    });
