...
type Ad = { id: string; imageUrl: string; linkUrl: string | null; active: boolean; sortOrder: number; displayType: string; title?: string | null };

export default function SuperAdminAdsPage() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [ads, setAds] = useState<Ad[]>([]);
  const [form, setForm] = useState({ imageUrl: "", linkUrl: "", displayType: "LANDING", title: "", description: "", ctaLabel: "" });
  ...
  async function add() {
    ...
    setForm({ imageUrl: "", linkUrl: "", displayType: "LANDING", title: "", description: "", ctaLabel: "" });
    load();
  }
  ...
  return (
    <div className="min-h-screen p-4 max-w-md mx-auto">
      ...
      <div className="bg-surface border border-surface2 rounded-xl p-4 mb-6">
        ...
        <input className="w-full bg-surface2 rounded-lg px-3 py-2 text-sm mb-2" placeholder="عنوان کمپین"
          value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
        <textarea className="w-full bg-surface2 rounded-lg px-3 py-2 text-sm mb-2" placeholder="توضیح کوتاه کمپین"
          value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
        <input className="w-full bg-surface2 rounded-lg px-3 py-2 text-sm mb-2" placeholder="متن دکمه؛ مثلاً مشاهده پیشنهاد"
        ...
        <select className="w-full bg-surface2 rounded-lg px-3 py-2 text-sm mb-3"
          ...
          <option value="LANDING">اسلایدر صفحه اصلی</option>
        </select>
      ...
      <div className="space-y-2">
        {ads.map((a) => (
          <div key={a.id} className="bg-surface2 border border-surface2 rounded-lg p-3 text-xs flex items-center gap-3">
            ...
            <div className="flex-1 truncate">{a.displayType === "POPUP" ? "پاپ‌آپ" : a.displayType === "LANDING" ? "اسلایدر اصلی" : "بنر"} · {a.title || a.linkUrl || "بدون عنوان"}</div>
