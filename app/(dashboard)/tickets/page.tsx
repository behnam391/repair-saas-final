...
function ActiveRepairTable({ tickets, onOpen }: { tickets: Ticket[]; onOpen: (ticket: Ticket) => void }) {
  ...
  return <section className={`repair-active-table ${tickets.length ? "" : "is-empty"}`}><header><div><b>تعمیرات فعال</b><small>{tickets.length ? "آخرین پذیرش‌های در جریان" : "هنوز تعمیر فعالی ثبت نشده است"}</small></div><span>{tickets.length.toLocaleString("fa-IR")} مورد اخیر</span></header>
